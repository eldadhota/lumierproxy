...
func handleUpdateDeviceAPI(w http.ResponseWriter, r *http.Request) {
	...
	oldUsername := ""
	if device != nil {
		oldUsername = device.Username
		// Prevent duplicate usernames
		newUsername := strings.TrimSpace(req.Username)
		if newUsername != "" {
			if existing := server.findDeviceByUsername(newUsername); existing != nil && existing != device {
				server.mu.Unlock()
		...
		// Update username and device map key if it changed
		if newUsername != oldUsername {
			...
			if newUsername != "" {
				...
				server.devices[newUsername] = device
			} else {
				device.Name = fmt.Sprintf("Anonymous-%s", device.IP)
				server.devices[device.IP] = device
			}
		}
	}
	server.mu.Unlock()
	...
	// Save config by Username if available, otherwise by IP
	server.persistMu.Lock()
	oldConfigKey := device.IP
	if oldUsername != "" {
		oldConfigKey = oldUsername
	}
	config := server.persistentData.DeviceConfigs[oldConfigKey]

	newConfigKey := oldConfigKey
	if strings.TrimSpace(req.Username) != oldUsername {
		delete(server.persistentData.DeviceConfigs, oldConfigKey)
		if device.Username != "" {
